import React from "react";
import Navigation from "../components/Navigation";

const Faq = () => {
  return (
    <div className="faq">
      <Navigation />
      <h2>Which is your favorite Pokemon ?</h2>
      <h3>
        I'd say Pikachu because of personal reasons but since it too obvious of
        a choice i'd pick Snorlax.
      </h3>
      <h2>Why did you create this site ?</h2>
      <h3>To learn Javascript and more precisely React.</h3>
      <h2>How was your day ?</h2>
      <h3>Quite good actually, seen some friends, pet my dog.</h3>
      <h2>Do you play Pokemon Go ?</h2>
      <h3>I do actually, and have been since the beginning pretty much.</h3>
      <h2>What is your favorite color ?</h2>
      <h3>Some shade of red.</h3>
      <h2>Do you think the human race has a future somehow ?</h2>
      <h3>Not really, we were doomed to kill ourselves from the beginning.</h3>
      <h2>Which is your favorite Pokemon ?</h2>
      <h3>Mewtoo.</h3>
    </div>
  );
};

export default Faq;
